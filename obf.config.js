...
// ─────────────────────────────────────────────────────────────
module.exports = {
  presets: { light: LIGHT, medium: MEDIUM, heavy: HEAVY },
  ...
  files: [
    ...
    // ── result.html (땡큐페이지) ──
    {
      ...
      preset: 'light',
      reason: 'JSONP 동적 콜백명 window[cbName]. CF-flattening 시 참조 깨짐 → light.'
    },
    {
    ...
    },
    {
      ...
      preset: 'light',
      reason: '중첩 클로저(forEach>addEventListener>di 캡처) 다수. medium CF-flattening이 인덱스 캡처/클릭 리스너 깨뜨려 카드 클릭 불가 → light. (UI 로직이라 노출 손해 적음)'
    },
    {
